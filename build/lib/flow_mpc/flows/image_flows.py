...
from .splits_and_priors import SplitFlow, SqueezeFlow
...
class GatedConv(nn.Module):
    ...
    def __init__(self, c_in, c_hidden):
        ...
        self.net = nn.Sequential(
            ...
            ConcatELU(),
            nn.Conv2d(2 * c_hidden, 2 * c_in, kernel_size=(1, 1))
        )
...
class GatedConvNet(nn.Module):
    ...
    def __init__(self, c_in, c_hidden=32, c_out=-1, num_layers=3):
        ...
        layers = []
        layers += [nn.Conv2d(c_in, c_hidden, kernel_size=(3, 3), padding=1)]
        for layer_index in range(num_layers):
            layers += [GatedConv(c_hidden, c_hidden),
                       LayerNormChannels(c_hidden)]
        layers += [ConcatELU(),
                   nn.Conv2d(2 * c_hidden, c_out, kernel_size=(3, 3), padding=1)]
...
class CouplingLayer(nn.Module):
    ...
    def forward(self, z, logpx, reverse=False, orig_img=None):
        ...
        return z, logpx + s.sum(dim=[1, 2, 3])
    ...
    def __init__(self, imsize=(64, 64)):
        ...
        for i in range(2):
            flow_chain.append(CouplingLayer(network=GatedConvNet(c_in=1, c_hidden=32),
                                            mask=create_checkerboard_mask(h=self.imsize[0], w=self.imsize[1],
                                                                          invert=(i % 2 == 1)),
                                            c_in=1))

        # Squeezes 1 x 64 x 64 -> 4 x 32 x 32
        ...
        # Squeezes 4 x 32 x 32 -> 16 x 16 x 16
        flow_chain.append(SqueezeFlow())
        # Splits to 8 x 16 x 16
        flow_chain.append(SplitFlow())

        for i in range(4):
            flow_chain.append(CouplingLayer(network=GatedConvNet(c_in=8, c_hidden=64),
